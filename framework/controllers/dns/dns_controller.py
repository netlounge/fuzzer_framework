...
class DnsController(BaseController):
    ...
    def __init__(self, name, host, port, logger=None):
        ...
        '''
        super(DnsController, self).__init__(name, logger)
        self._host = host
        self._port = port
        self._server = None
        self._active = False
    ...
    def setup(self):
        super(DnsController, self).setup()
    ...
    def teardown(self):
        ...
        :return:
        """
        super(DnsController, self).teardown()
        if not self.is_victim_alive():
    ...
    def post_test(self):
        ...
        :return:
        """
        super(DnsController, self).post_test()
        if not self.is_victim_alive():
            if self._server:
    ...
    def pre_test(self, test_number):
        ...
        """
        self._restart_target()
        super(DnsController, self).pre_test(test_number)
    ...
    def is_victim_alive(self):
        """
        Method check the server Dns response code
        :return:
        """
        self._active = False
        try:
